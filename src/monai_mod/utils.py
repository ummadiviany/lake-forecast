...
def sliding_window_inference(
    ...
    **kwargs: Any,
    ...
    print("Here in sliding_window_inference")
    ...
    for slice_g in tqdm(range(0, total_slices, sw_batch_size)) if progress else range(0, total_slices, sw_batch_size):
        ...
        window_data = rearrange(window_data, 'b c h w t  -> b c t h w')
        seg_prob_out = predictor(window_data, *args, **kwargs)  # batched patch segmentation
        print(f'seg_prob_out.shape: {seg_prob_out.shape}')
        seg_prob_out = rearrange(seg_prob_out, 'b c t h w -> b c h w t')
